...
export class NoteCard extends Component {
  ...
  updateListItems = (newItem, remove) => {
    let updateListItems;
    if (remove) {
      updateListItems = [...this.state.listItems].filter(
  ...
  render() {
    const completedListItems = this.props.note.listItems
      .filter(item => {
    ...
    const uncompletedListItems = this.props.note.listItems
      .filter(item => {
        return !item.completed;
      })
      .map(filteredItem => (
        ...
        <article className="note-card" style={{ background: this.state.color }}>
          ...
          <div className="svg-container">
            <div className="color-svgs">
              ...
              <i
                data-color="#fcf296"
                className="fas fa-circle yellow"
                onClick={e => {
                  this.colorChange(e);
                }}
              />
            ...
            <div>
              <i className="fas fa-trash-alt" onClick={this.deleteNote} />
            </div>
          </div>
        </article>
      </NavLink>
